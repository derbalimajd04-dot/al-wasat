...
exports.handler = async (event) => {
  ...
  if (!process.env.RESEND_API_KEY) {
    console.error('newsletter-signup: RESEND_API_KEY not set');
    return {
      statusCode: 500,
      headers: { 'Content-Type': 'application/json' },
  ...
  if (process.env.RESEND_AUDIENCE_ID) {
    try {
      ...
      console.error('newsletter-signup: failed to add contact:', err.message);
    }
  }
...
<table width="100%" cellpadding="0" cellspacing="0" style="background:#0A0906;padding:48px 20px">
  ...
  <table width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#110F0C;border:1px solid rgba(201,169,110,0.12)">
    ...
    <tr><td style="padding:48px 48px 32px;text-align:center">
      ...
      <p style="margin:24px 0 0;font-size:15px;color:#BEB49A;line-height:1.9;font-family:Georgia,serif">We'll be in touch when the 2025 harvest arrives.</p>
      <p style="margin:12px 0 0;font-size:13px;color:#8B7355;line-height:1.8">No noise. No promotions. Only the harvest.</p>
    </td></tr>
    <tr><td style="padding:8px 48px 40px;text-align:center">
      <div style="width:60px;height:1px;background:rgba(201,169,110,0.25);margin:0 auto 32px"></div>
      <a href="https://al-wasat.co.uk/shop/" style="display:inline-block;font-family:'Helvetica Neue',Arial,sans-serif;font-size:10px;letter-spacing:4px;text-transform:uppercase;color:#C9A96E;text-decoration:none;border:1px solid rgba(201,169,110,0.35);padding:14px 32px">View the Collection</a>
    </td></tr>
    <tr><td style="padding:24px 48px;text-align:center;border-top:1px solid rgba(201,169,110,0.08)">
      ...
      <p style="margin:8px 0 0;font-size:10px;color:#2A2018">Extra Virgin Olive Oil &middot; Tunisia &middot; Since 1960</p>
    </td></tr>
  </table>
  </td></tr>
</table>
    ...
    console.error('newsletter-signup: failed to send confirmation:', err.message);
    return {
      statusCode: 500,
      headers: { 'Content-Type': 'application/json' },
